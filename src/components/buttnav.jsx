...
export const Buttnav = ({ isActive, setIsActive }) => {
  return (
    ...
    >
      <motion.div
        className="relative h-full w-full font-roslindale-reg text-2xl uppercase "
...
function PerspectiveText({label}) {
  return (
    <div className="w-full h-full flex items-center justify-center group-hover/butnav:[transform:rotateX(90deg)] transition-[transform] ease-[cubic-bezier(0.76,0,0.24,1)] duration-[0.75s] [transform-style:preserve-3d]">
      <p className="group-hover/butnav:-translate-y-full group-hover/butnav:opacity-0 transition-all ease-[cubic-bezier(0.76,0,0.24,1)] duration-[0.75s] [transform-style:preserve-3d]">{label}</p>
      <p className="absolute [transform:rotateX(-90deg)]  opacity-0 group-hover/butnav:opacity-100 transition-all ease-[cubic-bezier(0.76,0,0.24,1)] duration-[0.75s] [transform-style:preserve-3d]">{label}</p>
    </div>
  )
}